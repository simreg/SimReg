...
class Trainer:
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        ...
        if self._signature_columns is None:
            ...
            # EDIT
            if self.columns_to_keep is not None and description == 'training':
                self._signature_columns += self.columns_to_keep
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            elif self.args.use_sgd_optimizer:
                from torch.optim import SGD
                optimizer_cls = SGD
                optimizer_kwargs = {'nesterov': self.args.nesterov_momentum > 0, 'momentum': self.args.nesterov_momentum}
            else:
                optimizer_cls = torch.optim.Adamax if self.args.use_adamax else AdamW
                optimizer_kwargs = {
            ...
            if self.sharded_ddp == ShardedDDPOption.SIMPLE:
                ...
                logger.info(f'Using {optimizer_cls} optimizer')
    ...
    def train(
        ...
        **kwargs,
        ...
        # EDIT: support for HALF_EPOCH evaluation/saving strategy
        if args.evaluation_strategy == IntervalStrategy.HALF_EPOCH:
            args.evaluation_strategy = IntervalStrategy.STEPS
            args.eval_steps = num_update_steps_per_epoch // 2
        if args.save_strategy == IntervalStrategy.HALF_EPOCH:
            args.save_strategy = IntervalStrategy.STEPS
            args.save_steps = num_update_steps_per_epoch // 2

        if args.regularization_delay > 0:
            if args.regularization_delay < 1:
                args.regularization_delay = math.ceil(max_steps * args.regularization_delay)
