...
def only_bias_arg(reg_only_biased, bias_type: BiasType, enforce_sim: bool):
    ...
    bias_dir = {
        ...
        BiasType.HYPOTHESIS: 'hypothesis_bias_debiasing/0.6_confidence',
    ...
    if reg_only_biased:
        ...
        bias_dir = {
            BiasType.LEXICAL: 'lexical_bias_debiasing/reg_OB',
            BiasType.UNKNOWN: 'unknown_bias/reg_OB/0.8_confidence',
            BiasType.HYPOTHESIS: 'hypothesis_bias_debiasing/reg_OB/0.6_confidence'
        }[bias_type]
        if not enforce_sim and bias_type == BiasType.HYPOTHESIS:
...
def main():
    ...
    regularize_only_biased = False

    if regularize_only_grads:
        node_name = "galileo2"  # galileo2, newton2, nlp-a40-1, galileo1
    else:
        node_name = "plato2"  # plato1, plato2

    unbiased_models = {
        BiasType.LEXICAL: ('runs/extrensic_debiasing/lexical_bias/bert_64_remove_confident_correct/checkpoint-11678', 'EDBXE'),
        # BiasType.LEXICAL: ('runs/extrensic_debiasing/lexical_bias/bert_64_remove_confident_correct', 'EDB'),
        BiasType.HYPOTHESIS: ('runs/extrensic_debiasing/hypothesis_bias/bert_64_0.6_hypothesis_extrensic_debiasing/checkpoint-7044', 'EDBXE'),
    ...
    bias_dir, reg_only_biased_arg, reg_only_biased_identifier = only_bias_arg(reg_only_biased=regularize_only_biased, bias_type=bias_type, enforce_sim=enforce_sim)

    counter = 0
    for regularization_lambda in [10]:
        for regularization_method in ["abs_cos_cor"]:  # abs_cos_cor
