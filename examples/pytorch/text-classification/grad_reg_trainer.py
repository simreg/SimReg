...
class BaseGradRegTrainer(SimRegTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.bias_sampler is not None and not self.bias_sampler.bias_turn and self.logging_mode == 'train':
            ...
            sim_loss = self.compute_grad_sim_loss(inputs, outputs, weak_outputs, optimize_sim_loss=optimize_sim_loss, indices=batch_bias_indices)
    ...
    def compute_grad_sim_loss(self, inputs, main_outputs, weak_outputs, optimize_sim_loss, indices=None):
        sim_loss = None

        main_grads = torch.autograd.grad(
            main_outputs.loss,
            inputs=self.activations,
            create_graph=self.logging_mode == 'train' and optimize_sim_loss,
            retain_graph=self.logging_mode == 'train' and optimize_sim_loss
        )
